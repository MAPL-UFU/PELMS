...
void setup(void) {
  ...
  snprintf(text_to_print, LCD_WIDTH + 1, "PNRD %s", pelmsReader);
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  printf(start ? "Tópico: %s\tMensagem: %s\n":"Subscribe:\nTópico: %s\tMensagem: %s\n", msg_topic.c_str(), msg.c_str());
  ...
  if (status != msg || topic_status != msg_topic) {
    ...
    if (!strcmp(msg_topic.c_str(), "PELMS-MQTT/fire/len_place")) {
      ...
      start = true;
    } else if (!strcmp(msg_topic.c_str(), "PELMS-MQTT/fire/len_transition") && start) {
