...
def parse_pnml_file(file):
    ...
    for net_node in root.iter(xmlns+'net'):
        ...
        net.id = net_node.get('id') # Return the id value
        netnmnode = net_node.find('./'+xmlns+'name/'+xmlns+'text')

        if netnmnode is not None: #if net name in the file set net name of the node
             net.name = netnmnode.text
        else: #if not net name in the file set net.neme as net.id
             net.name = net.id

        # Parse transitions
        for transition_node in net_node.iter(xmlns+'transition'):
            transition = Transition()
            net.set_len_transition() #define the length of the transition dictionary
