...
def template_wifi_h(ssid, userName, passWord, anonymous, clientid, server, port, serial, topic, readerid=None, antennaid=None, position=None):
    ...
    with open(f'{pathName}/template_wifipassword.h', encoding='utf8') if sys.platform == 'win32' else open(f'{pathName}/template_wifipassword.h', "r") as file:
            finalText = '\n'.join(map(str, file.read().splitlines()))
